...
logger_http = logging.getLogger("http_logger")  # Keep only one assignment
...
from fastapi.responses import JSONResponse  # Corrected import
import logging
...
async def end_ride(headers, bot_slug, mission_slug):
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
            raise HTTPException(status_code=response.status_code, detail="Failed to end mission")
        return response.json()
...
async def control(request: Request):
    ...
    try:
        # Send command using a browser service (or any bot SDK)
        await browser_service.send_message(command)
        return JSONResponse(status_code=200, content={"message": "Command sent successfully"})
    except Exception as e:
        logger.error(f"Error sending control command '{command}': {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to send control command") from e
    ...
    try:
        ...
        logger.error(f"Failed to send command '{command}' to bot, error: {str(e)}")
        raise HTTPException(
...
async def get_screenshot_v2():
    ...
    async def get_frame(frame_type):
        try:
            ...
            return {f"{frame_type}_frame": None}
    ...
    response_data = {key: value for result in results for key, value in result.items()}

    if not response_data.get("rear_frame") and not response_data.get("front_frame"):
...
async def get_front_frame():
    ...
    if front_frame:
        ...
        logger.warning("Front frame not available")
...
async def get_rear_frame():
    ...
    try:
        ...
        if rear_frame:
            ...
            logger.warning("Rear frame not available")
            raise HTTPException(status_code=404, detail="Rear frame not available")
    except Exception as e:
        logger.error(f"Error fetching rear frame: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal server error while fetching rear frame")
